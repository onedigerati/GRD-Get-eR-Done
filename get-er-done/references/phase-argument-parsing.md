...
```bash
PHASE_INFO=$(node ~/.claude/get-er-done/bin/grd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-er-done/bin/grd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-er-done/bin/grd-tools.js find-phase "${PHASE}" --raw)
```
