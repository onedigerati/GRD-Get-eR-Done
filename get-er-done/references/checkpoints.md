...
**Key distinction:** Auth gates are created dynamically when Claude encounters auth errors. NOT pre-planned â€” Claude automates first, asks for credentials only when blocked.
</type>
</checkpoint_types>
...
**Auth gate = Claude tried CLI/API, got auth error.** Not a failure â€” a gate requiring human input to unblock.

**Pattern:** Claude tries automation â†’ auth error â†’ creates checkpoint:human-action â†’ user authenticates â†’ Claude retries â†’ continues
...
**Protocol:** Try command â†’ "command not found" â†’ auto-installable? â†’ yes: install silently, retry â†’ no: checkpoint asking user to install.
...
**DON'T:**
- Ask human to do work Claude can automate âŒ
- Assume knowledge: "Configure the usual settings" âŒ
- Skip steps: "Set up database" (too vague) âŒ
- Mix multiple verifications in one checkpoint âŒ
...
**Bad placement:** Before automation âŒ | Too frequent âŒ | Too late (dependent tasks already needed the result) âŒ
</writing_guidelines>
...
### âŒ BAD: Asking user to start dev server
...
### âŒ BAD: Asking human to deploy / âœ… GOOD: Claude automates

```xml
<!-- BAD: Asking user to deploy via dashboard -->
<task type="checkpoint:human-action" gate="blocking">
  <action>Deploy to Vercel</action>
  <instructions>Visit vercel.com/new â†’ Import repo â†’ Click Deploy â†’ Copy URL</instructions>
</task>
...
### âŒ BAD: Too many checkpoints / âœ… GOOD: Single checkpoint
...
### âŒ BAD: Vague verification / âœ… GOOD: Specific steps
...
### âŒ BAD: Asking user to run CLI commands
...
### âŒ BAD: Asking user to copy values between services

```xml
<task type="checkpoint:human-action">
  <action>Configure webhook URL in Stripe</action>
  <instructions>Copy deployment URL â†’ Stripe Dashboard â†’ Webhooks â†’ Add endpoint â†’ Copy secret â†’ Add to .env</instructions>
</task>
```
