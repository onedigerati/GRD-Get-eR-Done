...
Detect whether GRD is installed locally or globally by checking both locations:

```bash
# Check local first (takes priority)
if [ -f "./.claude/get-er-done/VERSION" ]; then
  cat "./.claude/get-er-done/VERSION"
  echo "LOCAL"
elif [ -f ~/.claude/get-er-done/VERSION ]; then
  cat ~/.claude/get-er-done/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
```bash
npm view get-er-done-cc version 2>/dev/null
```
...
To update manually: `npx get-er-done-cc --global`
```
...
âš ï¸  **Note:** The installer performs a clean install of GRD folders:
- `commands/grd/` will be wiped and replaced
- `get-er-done/` will be wiped and replaced
...
- Custom commands not in `commands/grd/` âœ“
- Custom agents not prefixed with `grd-` âœ“
- Custom hooks âœ“
...
**If LOCAL install:**
```bash
npx get-er-done-cc --local
```

**If GLOBAL install (or unknown):**
```bash
npx get-er-done-cc --global
```
...
â•‘  GRD Updated: v1.5.10 â†’ v1.5.15                           â•‘
