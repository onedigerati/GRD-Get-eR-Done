...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node ~/.claude/get-er-done/bin/grd-tools.js phase insert "${after_phase}" "${description}")
```
