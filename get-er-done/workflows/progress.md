...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init progress --include state,roadmap,project,config)
```
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node ~/.claude/get-er-done/bin/grd-tools.js summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node ~/.claude/get-er-done/bin/grd-tools.js progress bar --raw)
```
...
- `/grd:list-phase-assumptions {phase}` â€” see Claude's assumptions
...
- `/grd:verify-work {phase}` â€” run more UAT testing
...
- `/grd:verify-work {Z}` â€” user acceptance test before continuing
...
**Also available:**
- `/grd:verify-work` â€” user acceptance test before completing milestone
...
- Phase complete but next phase not planned â†’ offer `/grd:plan-phase [next]`
- All work complete â†’ offer milestone completion
- Blockers present â†’ highlight before offering to continue
- Handoff file exists â†’ mention it, offer `/grd:resume-work`
  </step>
