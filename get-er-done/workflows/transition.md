...
```bash
TRANSITION=$(node ~/.claude/get-er-done/bin/grd-tools.js phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ~/.claude/get-er-done/bin/grd-tools.js progress bar --raw)
```
...
**Also available:**
- `/grd:discuss-phase [X+1]` â€” gather context first
- `/grd:research-phase [X+1]` â€” investigate unknowns
- Review roadmap
