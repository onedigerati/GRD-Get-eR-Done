...
1. **grd-phase-researcher** â€” Reads CONTEXT.md to know WHAT to research
...
2. **grd-planner** â€” Reads CONTEXT.md to know WHAT decisions are locked
...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init phase-op "${PHASE}")
```
...
**Also available:**
- `/grd:plan-phase ${PHASE} --skip-research` â€” plan without research
...
```bash
node ~/.claude/get-er-done/bin/grd-tools.js commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
