...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init quick "$DESCRIPTION")
```
...
**If `roadmap_exists` is false:** Error â€” Quick mode requires an active project with ROADMAP.md. Run `/grd:new-project` first.
...
```bash
node ~/.claude/get-er-done/bin/grd-tools.js commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${QUICK_DIR}/${next_num}-PLAN.md ${QUICK_DIR}/${next_num}-SUMMARY.md .planning/STATE.md
```
