...
<template>
@~/.claude/get-er-done/templates/UAT.md
</template>
...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init verify-work "${PHASE_ARG}")
```
...
Commit the UAT file:
```bash
node ~/.claude/get-er-done/bin/grd-tools.js commit "test({phase}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase}-UAT.md"
```
...
- `/grd:plan-phase {next}` â€” Plan next phase
- `/grd:execute-phase {next}` â€” Execute next phase
```
</step>
...
- Follow @~/.claude/get-er-done/workflows/diagnose-issues.md
