...
const banner = '\n' +
  ...
  '   ╚═════╝ ╚═╝  ╚═╝╚═════╝' + reset + '\n' +
  '\n' +
  '  Get eR Done ' + dim + 'v' + pkg.version + reset + '\n' +
  '  A meta-prompting, context engineering and spec-driven development system\n' +
  '  for Claude Code. Rebranded and enhanced with persistent Agent Teams for\n' +
...
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/grd-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to grd-statusline.js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'grd-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to grd-statusline.js in v1.9.0
    'grd-intel-index.js',  // Removed in v1.9.2
    'grd-intel-session.js',  // Removed in v1.9.2
    'grd-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  if (settings.statusLine && settings.statusLine.command &&
      ...
      !settings.statusLine.command.includes('grd-statusline.js')) {
    // Replace old path with new path
    settings.statusLine.command = settings.statusLine.command.replace(
      /statusline\.js/,
      'grd-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (statusline.js → grd-statusline.js)`);
  }
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    // Claude Code & Gemini: remove commands/grd/ directory
  ...
  // 2. Remove get-er-done directory
  const grdDir = path.join(targetDir, 'get-er-done');
  if (fs.existsSync(grdDir)) {
    fs.rmSync(grdDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed get-er-done/`);
  }
 ...
 * This prevents permission prompts when GRD accesses the get-er-done directory
 */
function configureOpencodePermissions() {
  ...
  const grdPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/get-er-done/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/get-er-done/*`;
...
const PATCHES_DIR_NAME = 'grd-local-patches';
const MANIFEST_NAME = 'grd-file-manifest.json';
...
function writeManifest(configDir) {
  const grdDir = path.join(configDir, 'get-er-done');
  const commandsDir = path.join(configDir, 'commands', 'grd');
  const agentsDir = path.join(configDir, 'agents');
  const manifest = { version: pkg.version, timestamp: new Date().toISOString(), files: {} };

  const grdHashes = generateManifest(grdDir);
  for (const [rel, hash] of Object.entries(grdHashes)) {
    manifest.files['get-er-done/' + rel] = hash;
  }
  if (fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/grd/' + rel] = hash;
    }
  }
  if (fs.existsSync(agentsDir)) {
...
/**
 * Detect user-modified GRD files by comparing against install manifest.
 * Backs up modified files to grd-local-patches/ for reapply after update.
 */
function saveLocalPatches(configDir) {
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified GRD file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
...
function reportLocalPatches(configDir) {
  ...
  if (meta.files && meta.files.length > 0) {
    ...
    console.log('  Run ' + cyan + '/grd:reapply-patches' + reset + ' to merge them into the new version.');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Save any locally modified GRD files before they get wiped
  ...
  // Copy get-er-done skill with path replacement
  const skillSrc = path.join(src, 'get-er-done');
  const skillDest = path.join(targetDir, 'get-er-done');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime);
  if (verifyInstalled(skillDest, 'get-er-done')) {
    console.log(`  ${green}✓${reset} Installed get-er-done`);
  } else {
    failures.push('get-er-done');
  }
  ...
  const changelogDest = path.join(targetDir, 'get-er-done', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'get-er-done', 'VERSION');
