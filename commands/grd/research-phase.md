...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init phase-op "$ARGUMENTS")
```
...
```bash
RESEARCHER_MODEL=$(node ~/.claude/get-er-done/bin/grd-tools.js resolve-model grd-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node ~/.claude/get-er-done/bin/grd-tools.js roadmap get-phase "${phase_number}")
```
