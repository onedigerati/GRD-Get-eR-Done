...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node ~/.claude/get-er-done/bin/grd-tools.js verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using grd-tools:

```bash
MUST_HAVES=$(node ~/.claude/get-er-done/bin/grd-tools.js frontmatter get "$PLAN_PATH" --field must_haves)
```
...
Use grd-tools plan-structure verification (already run in Step 2):

```bash
PLAN_STRUCTURE=$(node ~/.claude/get-er-done/bin/grd-tools.js verify plan-structure "$PLAN_PATH")
```
...
**For manual validation of specificity** (grd-tools checks structure, not content quality):
```bash
