...
```bash
node ~/.claude/get-er-done/bin/grd-tools.js commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init plan-phase "${PHASE}")
```
...
```bash
node ~/.claude/get-er-done/bin/grd-tools.js history-digest
```
...
```bash
VALID=$(node ~/.claude/get-er-done/bin/grd-tools.js frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ~/.claude/get-er-done/bin/grd-tools.js verify plan-structure "$PLAN_PATH")
```
...
```bash
node ~/.claude/get-er-done/bin/grd-tools.js commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
