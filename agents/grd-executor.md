...
```bash
INIT=$(node ~/.claude/get-er-done/bin/grd-tools.js init execute-phase "${PHASE}")
```
...
**See @~/.claude/get-er-done/references/checkpoints.md**
...
**Use template:** @~/.claude/get-er-done/templates/summary.md
...
<final_commit>
```bash
node ~/.claude/get-er-done/bin/grd-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md
```
